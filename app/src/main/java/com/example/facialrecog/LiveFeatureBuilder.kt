...
object LiveFeatureBuilder {
    ...
    fun build(
        ...
        val faceHeight = faceContour
            ?.let { pts -> pts.maxOf { p -> p.y } - pts.minOf { p -> p.y } }
            ?.takeIf { it > 0f } ?: 1f

        val rawMouthOpen =
            if (mouthPoints != null && upperLip != null &&
        ...
        val mouthCurve =
            if (mouthPoints != null && mouthPoints.size >= 3) {
                val lc = mouthPoints.first().y
                val rc = mouthPoints.last().y
                val cy = mouthPoints[mouthPoints.size / 2].y
                ((lc + rc) / 2f - cy) / faceHeight
            } else 0f

        val browRaise =
            if (leftBrow != null && rightBrow != null && faceContour != null) {
